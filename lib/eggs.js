...
var parseDirectives = function(model,el,parentTag){
  ...
  elements.forEach(function(element){
    ...
    if (nonParsed && select(nonParsed,el).length) {
...
// parses and returns html from our root node.
var html = function(){
...
var factory = function(htmlString,options,ViewModel,callback){
  ...
  eggs.html = html.bind(eggs);

  // set up our virtualDom for this html.
  try {
    eggs.rootNode = virtualize(htmlString);
  } catch(e){
  ...
  var tree = select(selector)(eggs.rootNode);
  console.log(tree[0],stringify(tree[0]));
  if (!tree) {
