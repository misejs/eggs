...
var parseDirectives = function(model,el,parentTag,callback){
  var self = this;
  ...
  elements.each(function(index,el){
    ...
    async.forEachSeries(runDirectives,callback);
  });
};
...
var modelUpdated = function(viewModel,changes){
  var self = this;
  var $ = self.$;
  // TODO: this should only modify the vdom when updating, and return something we can diff.
  vdom.render($,function(callback){
    var elements = $(self.selector).toArray();
    async.forEach(elements,function(element,done){
      parseDirectives.call(self,viewModel,element,null,done);
    },callback);
  });
}
